...
BioLip = '/data/Mukuo/Aurora_B/L-score/EVis-main/BioLip/'  ### update
...
command1 = 'cd ' + rootdir +'/user_input/results_docking/{}/'.format(sys.argv[1])

total_mol2 = list()
lsalign_index = 0
for mol2_item in (os.listdir(rootdir +'/user_input/results_docking/{}/'.format(sys.argv[1]))):
...
command= scriptdir + '/L_score.py {}'.format(sys.argv[1])
stdout,stderr = subprocess.Popen(';'.join([command]),shell=True).communicate()
