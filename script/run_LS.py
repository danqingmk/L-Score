...
BioLip = '/home/jianping/data/wanglin/EViS-main/BioLip/'  ### update
...
#
 ...
 stdout,stderr = subprocess.Popen(';'.join([command]),shell=True).communicate()
#
 command = 'cp ' + rootdir +'/user_input/results_docking/pocket/pocket_topN* ' + rootdir +'/user_input/results_docking/ligand_mol2/'
 stdout,stderr = subprocess.Popen(';'.join([command]),shell=True).communicate()
#
...
command1 = 'cd ' + rootdir +'/user_input/results_docking/ligand_mol2/'

total_mol2 = list()
lsalign_index = 0
for mol2_item in (os.listdir(rootdir +'/user_input/results_docking/ligand_mol2/')):
...
#### step 4: PL-score
print ("step4/4 run L-score")
command= scriptdir + '/L_score.py'
stdout,stderr = subprocess.Popen(';'.join([command]),shell=True).communicate()
